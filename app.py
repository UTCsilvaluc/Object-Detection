...
def add_rectangle():
    ...
    def to_py(obj):
        if isinstance(obj, np.generic):
            ...
            return {to_py(key): to_py(value) for key, value in obj.items()}
        return obj
    result_data = to_py(result_data)
...
def merge_objects():
    ...
    print(f"Received request to merge objects: {object_ids} in image {img_name}")
    ...
    with open(json_file_path, 'r', encoding='utf-8') as json_file:
        result_data = json.load(json_file)
    objects = result_data.get("objects", [])
    # Convert objetst_ids to integers for comparison
    ...
    image = cv2.imread(img_original_path)
    new_id = (objects[-1]["id"] + 1) if objects else 1
    ...
    print(f"Les contours sont : {contours}")
    """
    ...
    return {"success": True , "num_objects": int(len(result_data["objects"])) , "img_annotated_path": str(img_annotated_path) , "nameNewObj":str(temp_object_name) , "pathObj":str(path) , "bbox":str(bbox) , "id":int(new_id)}, 200
